    ...
    <div class="container">
        ...
        <div id="item_table" class="section">
            ...
            <div class="accordion" id="item_accordion">
                @forelse($items_categorized_array as $array)
                    <div class="card item-{{ $array['category']->name }}-table">
                        <div class="card-header" id="item-{{ $array['category']->name }}-header">
                            <h5 class="mb-0">
                                <button class="btn btn-link" type="button" data-bs-toggle="collapse" data-bs-target="#item-{{ $array['category']->name }}-collapse" aria-expanded="true" aria-controls="item-{{ $array['category']->name }}-collapse">
                                    カテゴリー：{{ $array['category']->name }}
                                </button>
                            </h5>
                        </div>
                        <div id="item-{{ $array['category']->name }}-collapse" class="collapse" aria-labelledby="item-{{ $array['category']->name }}-header" data-bs-parent="#item_accordion">
                            <div class="card-body">
